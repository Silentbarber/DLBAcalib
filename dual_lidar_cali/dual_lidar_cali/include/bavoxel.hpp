...
int layer_limit = 3;
int layer_size[] = {20, 20, 20, 30,30};//{30, 30, 30, 30}
//  float eigen_value_array[] = {1.0/4.0, 1.0/4.0, 1.0/4.0};
float thresh_num = 1.0/10;
...
struct cloud_property{
    int point_density_midle;
    int low_point_num;
    double low_ratio ;
};
struct DDBC{
    vector<PLV(3)> static_cloud;
    vector<PLV(3)> dynamic_cloud;
};
class VOX_HESS
{
public:
  ...
  void acc_evaluate2(const vector<IMUST> &xs, int head, int end, Eigen::MatrixXd &Hess, Eigen::VectorXd &JacT, double &residual)
  {
    ...
    for(int a=head; a<end; a++)
    {
      ...
      residual += coe * lmbd[kk];//each voxel point num * each feature min eigenvalue
    }
  ...
  void left_evaluate(const vector<IMUST> &xs, int head, int end, Eigen::MatrixXd &Hess, Eigen::VectorXd &JacT, double &residual)
  {
    ...
    for(int a=head; a<end; a++)
    {
      ...
      residual += coe * lmbd[l];//each voxel point num * each feature min eigenvalue
...
public:
  ...
  double density;
  float similar_score;
  vector<vector<PLV(3)>> simple_cloud_cluster;
  unordered_map<int,unordered_map<int, PLV(3)>> dbscan_result;
  vector<float> cluster_score;
  vector<int> time_series;
  cloud_property voxel_property;
    DDBC ddbc;
  ...
  OCTO_TREE_NODE()
  {
    ...
    dynami_score=0;//means no point
      is_plane=0;
      is_dynamic=0;//0 unknow 1 dynamic 2 static
      similar_score=0;
      density=1.0;
      voxel_property.low_point_num=0;
      voxel_property.low_ratio=0;
      voxel_property.point_density_midle=0;
  ...
  void hash2vector()
  {
      ...
      for(auto cluster = dbscan_result.begin(); cluster != dbscan_result.end(); cluster++)
      {
          vector<PLV(3)> each_cluster(win_size);
          for(auto frame = cluster->second.begin(); frame != cluster->second.end(); frame++)
          {
              each_cluster[frame->first]=frame->second;
          }
          simple_cloud_cluster.push_back(each_cluster);
      }
      dbscan_result.clear();
  }
  ...
  void subsample_voxel(int win_count)
  {
      if(octo_state != 1)
      {
          int point_size = fix_point.N;
          for(int i=0; i<win_count; i++)
              point_size += sig_tran[i].N;
...
void cut_voxel_one_time(unordered_map<VOXEL_LOC, OCTO_TREE_ROOT*> &feat_map, pcl::PointCloud<PointType> &pl_feat, const IMUST &x_key, int fnum)
{
    float loc_xyz[3];
    ...
    {
        Eigen::Vector3d pvec_orig(p_c.x, p_c.y, p_c.z);
        Eigen::Vector3d pvec_tran = pvec_orig;
