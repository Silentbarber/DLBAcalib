...
int layer_limit = 4;//最大层数
int layer_size[] = {30, 30, 30, 30,30};//{30, 30, 30, 30}
//  float eigen_value_array[] = {1.0/4.0, 1.0/4.0, 1.0/4.0};
float thresh_num = 1.0/20;
float eigen_value_array[5] = {thresh_num, thresh_num, thresh_num, thresh_num,thresh_num};
int min_ps = 10;//节点内最少包含的点数
...
public:
  ...
  {
    static double cos1 = cos(8/57.3);//==cos(8)
    static double cos2 = cos(80/57.3);//==cos
    ...
    {
      Eigen::Vector3d c2 = centers[i];//  取出每个节点中心 和法向量
...
public:
  ...
  OCTO_TREE_NODE()
  {
    ...
    for(int i=0; i<8; i++) leaves[i] = nullptr;//每个体素下的八个子体素
    ref = 255.0*rand()/(RAND_MAX + 1.0f);
    layer = 0;
  }

  bool judge_eigen(int win_count)//判断体素内是否只有一个特征
  {
    PointCluster covMat = fix_point;
    for(int i=0; i<win_count; i++)
      covMat += sig_tran[i];
    
    Eigen::SelfAdjointEigenSolver<Eigen::Matrix3d> saes(covMat.cov());//该体素叶节点内的协方差矩阵
    value_vector = saes.eigenvalues();//协方差矩阵特征值
    center = covMat.v / covMat.N;//取重心
    direct = saes.eigenvectors().col(0);//取法向量

    decision = saes.eigenvalues()[0] / saes.eigenvalues()[1];//最小特征值与第二小特征值进行比较，若比值很小，则认为只有一个平面特征

    // decision = saes.eigenvalues()[0];

    double eva0 = saes.eigenvalues()[0];//第一个特征值
    ...
    for(int i=0; i<8; i++)
    {
      if(covMats[i].N < 10) continue;//子节点点数小于10就不再分割
  ...
  void cut_func(int ci)
  {
    ...
    PLV(3) &pvec_tran = vec_tran[ci];//每一帧下面的点
  ...
  void recut(int win_count)
  {
    if(octo_state != 1)
    {
      int point_size = fix_point.N;//当前点簇中点的数量
      for(int i=0; i<win_count; i++)
        point_size += sig_orig[i].N;//遍历当前体素下每一帧的点簇，得到每一帧的
...
public:
  ...
  Eigen::VectorXd damping_iter(vector<IMUST> &x_stats, VOX_HESS &voxhess)
  {
    double u = 0.01, v = 2;
...
//    cout<<"H    :   "<<Hess.diagonal()(0)<<endl;
//    double dd = Hess.inverse().diagonal()(0);
//    Eigen::VectorXd Hs = Hess.inverse().diagonal();
      Eigen::VectorXd Hs(Hess.diagonal().size());
      cout<<"H::"<<Hess.diagonal().size()<<endl;
      for (int i = 0; i < Hess.diagonal().size(); ++i) {
          Hs [i] =  double (Hess.diagonal()(i));
      }
...
void cut_voxel(unordered_map<VOXEL_LOC, OCTO_TREE_ROOT*> &feat_map, pcl::PointCloud<PointType> &pl_feat, const IMUST &x_key, int fnum)//八叉树地图（大体素，内部划分的八个小体素） ，对应窗口点云，对应位姿，对应帧
{
  float loc_xyz[3];

  int ii=0;
  for(PointType &p_c : pl_feat.points)//提取一帧点云里的每个点
  {
    Eigen::Vector3d pvec_orig(p_c.x, p_c.y, p_c.z);//提取xyz
    Eigen::Vector3d pvec_tran = x_key.R*pvec_orig + x_key.p;//位姿变换
    ...
    VOXEL_LOC position((int64_t)loc_xyz[0], (int64_t)loc_xyz[1], (int64_t)loc_xyz[2]);//取Int，就是划分以米为单位的体素
    auto iter = feat_map.find(position);
    if(iter != feat_map.end())//若该position存在于哈系表中
    {
      
      if(iter->second->octo_state != 2)//不是平面特帧
      {
    ...
    {
      OCTO_TREE_ROOT *ot = new OCTO_TREE_ROOT();//对于每一帧的每一个点都建立一个根结点
      ot->vec_orig[fnum].push_back(pvec_orig);//保存当前帧的点，下标表示第几帧
      ...
      ot->layer = 0;
      feat_map[position] = ot;//建立哈希映射 通过体素的位置访问对应的点
      ii++;
    }
